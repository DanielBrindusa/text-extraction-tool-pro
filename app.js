...
function notifyParentOfHeight() {
  try {
    ...
    window.parent.postMessage({ type: "textExtractionToolPro:resize", height }, "*");
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", scheduleHeightSync);



const __heightObserver = new MutationObserver(() => scheduleHeightSync());
window.addEventListener("DOMContentLoaded", () => {
  try {
    __heightObserver.observe(document.body, { childList: true, subtree: true, attributes: true, characterData: true });
    scheduleHeightSync();
  } catch (error) {
